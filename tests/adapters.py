...
from cs336_basics.train_bpe import train_bpe as my_train_bpe
from cs336_basics.tokenizer import Tokenizer as myTokenizer
...
def run_transformer_lm(
    ...
    """
    # raise NotImplementedError
    tlm = TransformerLM(vocab_size, d_model, num_layers, num_heads, d_ff, context_length, rope_theta)
    tlm.token_embeddings.weight.data = weights["token_embeddings.weight"]
    for i in range(num_layers):
        tlm.layer[i].attn.q_proj.weight.data = weights[f"layers.{i}.attn.q_proj.weight"]
        tlm.layer[i].attn.k_proj.weight.data = weights[f"layers.{i}.attn.k_proj.weight"]
        tlm.layer[i].attn.v_proj.weight.data = weights[f"layers.{i}.attn.v_proj.weight"]
        tlm.layer[i].attn.output_proj.weight.data = weights[f"layers.{i}.attn.output_proj.weight"]
        tlm.layer[i].ln1.weight.data = weights[f"layers.{i}.ln1.weight"]
        tlm.layer[i].ffn.w1.weight.data = weights[f"layers.{i}.ffn.w1.weight"]
        tlm.layer[i].ffn.w2.weight.data = weights[f"layers.{i}.ffn.w2.weight"]
        tlm.layer[i].ffn.w3.weight.data = weights[f"layers.{i}.ffn.w3.weight"]
        tlm.layer[i].ln2.weight.data = weights[f"layers.{i}.ln2.weight"]
    tlm.ln_final.weight.data = weights["ln_final.weight"]
    tlm.lm_head.weight.data = weights["lm_head.weight"]
...
def run_get_lr_cosine_schedule(
    ...
    """
    # raise NotImplementedError
    return get_lr_cosin_schedule(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
...
def get_tokenizer(
    ...
    """
    # raise NotImplementedError
    return myTokenizer(vocab, merges, special_tokens)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    """
    # raise NotImplementedError
    return my_train_bpe(input_path, vocab_size, special_tokens)
