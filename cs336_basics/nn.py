...
class TransformerLM(nn.Module):
    def __init__(self, vocab_size: int, d_model: int, num_layers: int, num_heads: int, d_ff: int, max_seq_len: int | None = None, theta: float | None = None):
        super().__init__()
        self.token_embeddings = Embedding(vocab_size, d_model)
        self.layer = nn.ModuleList([TransformerBlock(d_model, num_heads, d_ff, max_seq_len, theta) for _ in range(num_layers)])
...
def cross_entropy(logits: torch.Tensor, targets: torch.Tensor):
    ...
    target_loss = -torch.gather(shifted_logits, dim = -1, index = targets.unsqueeze(-1))
